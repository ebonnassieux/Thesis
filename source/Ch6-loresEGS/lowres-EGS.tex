\chapter{Imaging the full LOFAR field of view}\label{section.EGS.lowres}
\minitoc
% % % % % % % % % % % % % % % % % % % % % % %
...
A more complete sky model can be created from this $\sim5''$-resolution image, and %, allowing for one last round of self-calibration on the 3C295 high-resolution model. 
bright out-of-field sources can be subtracted from high-resolution images of the EGS so that their sidelobe emission do not contaminate the final images. %  This ensures that we do not ``hard-set" errors in our high-resolution model caused by other sources in the field. 
...
\pg
The data was retrieved from the LOFAR Long-Term Archive, and is thus pre-processed and flagged for RFI using the standard tools \citepads[NDPPP and AOFlagger; see][ respectively]{2018ascl.soft04003V,2010ascl.soft10017O}. The next step is therefore to calibrate the data.
...
\end{subfigure}
\caption{\label{image.egs.widefield.ddsc} Wide-field image cutouts of the full primary beam, centred on the EGS, before and after selfcal over multiple facets. Note the massive decrease in artefacts centred on 3C295, but these are not the only artefacts decreased. \cref{image.egs.widefield.ddsc.zoom} show zoomed-in cutouts of this field as proof that the calibration artefacts around fainter sources decrease as well as a consequence of direction-dependent calibration.}
\end{figure}
...
\caption{\label{image.egs.widefield.noddsc.zoom} LOFAR wide-field image cutout, calibrated with the initial sky model and no direction-dependent Jones terms.}
\end{subfigure}
\hfill
\begin{subfigure}{.8\textwidth}
...
\end{subfigure}
\caption{\label{image.egs.widefield.ddsc.zoom} Cutouts of smaller regions of \cref{image.egs.widefield.ddsc}, intended to show that calibration artefacts around fainter sources decrease as well as those around 3C295.}
\end{figure}
...
\section{Effect of Quality-based Weighting Scheme}
...
\resizebox{\hsize}{!}{\includegraphics{images/{./image_full_ampphase_di_m.NS_Smooth_noW.noise01.fitsnoisemap.weights.compare}.png}}
...
\hfill
\begin{subfigure}{.65\textwidth}
\resizebox{\hsize}{!}{\includegraphics{images/{./image_full_ampphase_di_m.NS_Smooth.noise01.fitsnoisemap.weights.compare}.png}}
...
\end{subfigure}
\caption{\label{image.egs.widefield.weights} Impact of quality-based weighting scheme on the final image. Note that \cref{image.egs.widefield.weighted} is the same image, albeit at a different contrast.} %%%here%%%
\end{figure}
...
% % % % % % % % % % % % % % % % % % % % % % %
\section{Zoo of Radio Sources}\label{sec.lowresEGS.overlays}

\subsection{Selected Sources}

\pg
We select 12 sources in the wide-field image, which is shown in \cref{plot.EGS.lofar.widefield}. Our selection is primarily based on whether sources have interesting or peculiar diffuse emission, though some are chosen for being particularly classic examples of radio galaxies. We scan the image from south to north and east to west (scanning upwards then shifting to the right in the image). The positions of the brightest sources ($>1$Jy)are given in \cref{table.egs.sources}. We insist that this is a very crude scientific analysis, aimed as a showcase for the reader of this manuscript.


\begin{figure}[h!]
\centering
\includegraphics[width=0.95\textwidth]{images/{lofar.widefield}.png}
\caption{\label{plot.EGS.lofar.widefield} Full image of the LOFAR primary beam, centred on the EGS. Image made without international stations and with direction-dependent calibration.}
\end{figure}
...
\end{subfigure}
\caption{\label{fig.egs1} Images \& Overlays for EGS-1. The overlays start at the $2\sigma$ level and end on the image emission peak.}
\end{figure}

\pg
EGS-1 was selected from the wide-field LOFAR image because it showed interesting diffuse structure. It does not seem to match up to any optical source, nor does it match an infrared source. EGS-1 therefore seems to be best identified as a low-frequency counterpart to NVSS J143706+533225. Its shape seems to include two jets, one much brighter than the other, but both visible in \cref{fig.egs1.lofarim}. This could include a radio galaxy with one Doppler-boosted jet, or asymmetry in its environment density. % The absence of an optical counterpart could be due to dust extinction or to insufficient sensitivity. 
...
\end{subfigure}
\caption{\label{fig.egs2} Images \& Overlays for EGS-2. The overlays start at the $5\sigma$ level and end on the image emission peak.}
\end{figure}
...
\end{subfigure}
\caption{\label{fig.egs3} Images \& Overlays for EGS-3. The overlays start at the $1.5\sigma$ level and end on the image emission peak.}
\end{figure}
...
\end{subfigure}
\caption{\label{fig.egs4} Images \& Overlays for EGS-4. The overlays start at the $2\sigma$ level and end on the image emission peak.}
\end{figure}

\pg
Another impressive source, EGS-4 was selected for its complex diffuse structure. Like EGS-3, the structure of this source seems to indicate that it would be a radio jet galaxy within a galaxy cluster. By the same argument as for EGS-3, we expect this diffuse emission to have associated optical emission somewhere along its central axis. As we can see from \cref{fig.egs4.sdss.nvssoverlay}, EGS-4 can be associated to an NVSS source: NVSS J143137+522728.  Its LOFAR structure matches the lower-resolution NVSS morphology, which is a strong indication that this structure is physical. This radio galaxy lies at redshift 0.292 \citepads{2010ApJ...723.1119L}.%, and is itself associated to the NVSS source in the middle of EGS-4 and its WISE counterpart. SIMBAD has no further association for this radio galaxy.
...
\end{subfigure}
\caption{\label{fig.egs6} Images \& Overlays for EGS-6. The overlays start at the $1.5\sigma$ level and end on the image emission peak.}
\end{figure}
...
\end{subfigure}
\caption{\label{fig.egs7} Images \& Overlays for EGS-7. The overlays start at the $1.5\sigma$ level and end on the image emission peak.}
\end{figure}
...
\end{subfigure}
\caption{\label{fig.egs8} Images \& Overlays for EGS-8. The overlays start at the $3\sigma$ level and end on the image emission peak.}
\end{figure}
...
\end{subfigure}
\caption{\label{fig.egs9} Images \& Overlays for EGS-9. The overlays start at the $1.5\sigma$ level and end on the image emission peak.}
\end{figure}
...
%\pg
%The southern source has a faint NVSS equivalent, but is still dominated by artefacts in the LOFAR image. We therefore do not seek to extract information from this source at the time of writing.
...
\end{subfigure}
\caption{\label{fig.egs10} Images \& Overlays for EGS-10. The overlays start at the $1.8\sigma$ level and end on the image emission peak.}
\end{figure}
...
\end{subfigure}
\caption{\label{fig.egs11} Images \& Overlays for EGS-11. The overlays start at the $2.5\sigma$ level and end on the image emission peak.}
\end{figure}
...
\end{subfigure}
\caption{\label{fig.egs12} Images \& Overlays for EGS-12. The overlays start at the $1.5\sigma$ level and end on the image emission peak.}
\end{figure}
...
\pg
The work done in this section had the primary goal of creating a decent model of the sky seen by LOFAR's primary beam. To do this, a wide-field, $6''$-resolution image of the fully primary beam was made: this image makes no use of the LOFAR international stations due to technical limitations. Using PyBDSM \citepads{2015ascl.soft02007M}, a sky model of the full field of view was extracted from the aforementioned image made after a few rounds of direction-dependent self-calibration.
